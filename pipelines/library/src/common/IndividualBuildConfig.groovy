...
class IndividualBuildConfig implements Serializable {
    ...
    IndividualBuildConfig(Map<String, ?> map) {
        ARCHITECTURE = map.get("ARCHITECTURE").trim()
        TARGET_OS = map.get("TARGET_OS").trim()
        VARIANT = map.get("VARIANT").trim()
        JAVA_TO_BUILD = map.get("JAVA_TO_BUILD").trim()
        ...
        SCM_REF = map.get("SCM_REF").trim()
        AQA_REF = map.get("AQA_REF").trim()
        AQA_AUTO_GEN = map.get("AQA_AUTO_GEN").trim()
        BUILD_ARGS = map.get("BUILD_ARGS").trim()
        NODE_LABEL = map.get("NODE_LABEL").trim()
        ADDITIONAL_TEST_LABEL = map.get("ADDITIONAL_TEST_LABEL").trim()
        KEEP_TEST_REPORTDIR = map.get("KEEP_TEST_REPORTDIR").trim()
        ACTIVE_NODE_TIMEOUT = map.get("ACTIVE_NODE_TIMEOUT").trim()
        CODEBUILD = map.get("CODEBUILD").trim()
        DOCKER_IMAGE = map.get("DOCKER_IMAGE").trim()
        DOCKER_FILE = map.get("DOCKER_FILE").trim()
        DOCKER_NODE = map.get("DOCKER_NODE").trim()
        DOCKER_REGISTRY = map.get("DOCKER_REGISTRY").trim()
        DOCKER_CREDENTIAL = map.get("DOCKER_CREDENTIAL").trim()
        PLATFORM_CONFIG_LOCATION = map.get("PLATFORM_CONFIG_LOCATION").trim()
        CONFIGURE_ARGS = map.get("CONFIGURE_ARGS").trim()
        OVERRIDE_FILE_NAME_VERSION = map.get("OVERRIDE_FILE_NAME_VERSION").trim()
        USE_ADOPT_SHELL_SCRIPTS = map.get("USE_ADOPT_SHELL_SCRIPTS").trim()
        ADDITIONAL_FILE_NAME_TAG = map.get("ADDITIONAL_FILE_NAME_TAG").trim()
        JDK_BOOT_VERSION = map.get("JDK_BOOT_VERSION").trim()
        RELEASE = map.get("RELEASE").trim()
        PUBLISH_NAME = map.get("PUBLISH_NAME").trim()
        ADOPT_BUILD_NUMBER = map.get("ADOPT_BUILD_NUMBER").trim()
        ENABLE_TESTS = map.get("ENABLE_TESTS").trim()
        ENABLE_TESTDYNAMICPARALLEL = map.get("ENABLE_TESTDYNAMICPARALLEL").trim()
        ENABLE_INSTALLERS = map.get("ENABLE_INSTALLERS").trim()
        ENABLE_SIGNER = map.get("ENABLE_SIGNER").trim()
        CLEAN_WORKSPACE = map.get("CLEAN_WORKSPACE").trim()
        CLEAN_WORKSPACE_AFTER = map.get("CLEAN_WORKSPACE_AFTER").trim()
        CLEAN_WORKSPACE_BUILD_OUTPUT_ONLY_AFTER = map.get("CLEAN_WORKSPACE_BUILD_OUTPUT_ONLY_AFTER").trim()
    }
